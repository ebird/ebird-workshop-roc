...
We've included a set of regional statistics for Chile and the regions of Chile in the data package provided for this workshop. We can use these statistics to identify which of the 2,882 Status and Trends species occur in Chile.
...
So, for the same total area, we can capture a much larger proportion of the populations of these species if we use eBird Status data products for these species in our site prioritization. This exampel is, of course, overly simplistic. In practices, systematic conservation planning tools like the R package [`prioritizr`](https://prioritizr.net/) can perform much more complex prioritizations, accounting for both the spatial and temporal aspects of species distributions, taking advantage of [site complementarity](https://link.springer.com/article/10.1007/BF02704970), allowing for more flexible conservation goals, and accounting for both conservation value and economic cost of sites. eBird Status data products are well suited for use in systematic conservation planning exercises using `prioritizr`.
